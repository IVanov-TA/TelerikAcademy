namespace MatrixClassTask
{
    using System;
    using System.Text;

    public class Matrix<T>
        where T : struct
    {
        #region FIELDS

        private T[,] matrix;
        private int rows;
        private int cols;

        #endregion

        #region CONSTRUCTORS

        public Matrix(int row, int col)
        {
            this.matrix = new T[this.Row = row, this.Col = col];
        }

        #endregion

        #region PROPERTIES

        public int Row
        {
            get
            {
                return this.rows;
            }

            private set
            {
                if (value < 2)
                {
                    throw new ArgumentException("Invalid rows value");
                }

                this.rows = value;
            }
        }

        public int Col
        {
            get
            {
                return this.cols;
            }

            private set
            {
                if (value < 2)
                {
                    throw new ArgumentException("Invalid column value");
                }

                this.cols = value;
            }
        }

        public T this[int row, int col]
        {
            get
            {
                if (row < 0 || row >= this.Row || col < 0 || col >= this.Col)
                {
                    throw new IndexOutOfRangeException("Index out of range");
                }

                return this.matrix[row, col];
            }

            set
            {
                if (row < 0 || row > this.Row || col < 0 || col > this.Col)
                {
                    throw new IndexOutOfRangeException("Index out of range");
                }

                this.matrix[row, col] = value;
            }
        }

        #endregion

        #region METHODS

        public static Matrix<T> operator +(Matrix<T> m1, Matrix<T> m2)
        {
            if (m1 == null || m2 == null)
            {
                throw new ArgumentNullException("Matrix {0} is null !", m1 == null ? "1" : "2");
            }

            if (m1.Row != m2.Row || m1.Col != m2.Col)
            {
                throw new ArgumentException("Matreces must have same dimensions when summing !");
            }

            Matrix<T> result = new Matrix<T>(m1.Row, m1.Col);

            for (int i = 0; i < m1.Row; i++)
            {
                for (int j = 0; j < m1.Col; j++)
                {
                    result[i, j] = (dynamic)m1[i, j] + (dynamic)m2[i, j];
                }
            }

            return result;
        }

        public static Matrix<T> operator -(Matrix<T> m1, Matrix<T> m2)
        {
            if (m1 == null || m2 == null)
            {
                throw new ArgumentNullException("Matrix {0} is null !", m1 == null ? "1" : "2");
            }

            if (m1.Row != m2.Row || m1.Col != m2.Col)
            {
                throw new ArgumentException("Matreces must have same dimensions when substracting !");
            }

            Matrix<T> result = new Matrix<T>(m1.Row, m1.Col);

            for (int i = 0; i < m1.Row; i++)
            {
                for (int j = 0; j < m1.Col; j++)
                {
                    result[i, j] = (dynamic)m1[i, j] - (dynamic)m2[i, j];
                }
            }

            return result;
        }

        public static Matrix<T> operator *(Matrix<T> m1, Matrix<T> m2)
        {
            if (m1 == null || m2 == null)
            {
                throw new ArgumentNullException("Matrix {0} is null !", m1 == null ? "1" : "2");
            }

            if (m1.Row != m2.Col)
            {
                throw new ArgumentException("Matrix 1 rows and Matrix 2 columns must be the same number !");
            }

            Matrix<T> result = new Matrix<T>(m1.Row, m2.Col);

            for (int row = 0; row < m1.Row; row++)
            {
                for (int col = 0; col < m2.Col; col++)
                {
                    dynamic temp = 0;
                    for (int currentnumbers = 0; currentnumbers < m1.Col; currentnumbers++)
                    {
                        temp = temp + ((dynamic)m1[row, currentnumbers] * (dynamic)m2[currentnumbers, col]);
                    }

                    result[row, col] = temp;
                }
            }

            return result;
        }

        public static bool operator true(Matrix<T> currmatrix)
        {
            for (int row = 0; row < currmatrix.Row; row++)
            {
                for (int col = 0; col < currmatrix.Col; col++)
                {
                    if (currmatrix[row, col] != (dynamic)0)
                    {
                        return true;
                    }
                }
            }

            return false;
        }

        public static bool operator false(Matrix<T> currmatrix)
        {
            for (int row = 0; row < currmatrix.Row; row++)
            {
                for (int col = 0; col < currmatrix.Col; col++)
                {
                    if (currmatrix[row, col] != (dynamic)0)
                    {
                        return true;
                    }
                }
            }

            return false;
        }

        public override string ToString()
        {
            StringBuilder toStringer = new StringBuilder();

            for (int i = 0; i < this.Row; i++)
            {
                for (int j = 0; j < this.Col; j++)
                {
                    toStringer.Append(this.matrix[i, j] + " ");
                }

                toStringer.Append(Environment.NewLine);
            }

            return toStringer.ToString();
        }

        #endregion
    }
}
